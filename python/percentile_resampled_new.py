...
bin_size       = 4       # size of percentile bins for histogram
turb_threshold = 200     # cut off turbidity values greater than
turb_min       = 0
adcp_threshold = 220    # cut off adcp intensities greater than
adcp_binrange  = [1,5]   # range of adcp bins to average (actual bins, not indices)
turb_fit_type  = 'lin'  # 'lin' or 'quad' or 'exp' or 'thrd'
adcp_fit_type  = 'exp'   # 'lin' or 'quad' or 'exp' or 'thrd'
limit_overlap  = 0       # 0 or 1 , limit time series to period of overlap

datastart = '2017-01-01 19:00:00'
datastop  = '2018-04-11 15:00:00'
# %% import pickle, faster than reading the xlsx

os.chdir('E:/Projects/Mobile/MobileISCO20170425_mike/')

adcp = pd.read_pickle('{}_hadcp_20170531.pkl'.format(site_name))
isco = pd.read_pickle('{}_isco_20170531.pkl'.format(site_name))
turb = pd.read_pickle('{}_turb_20170531.pkl'.format(site_name))
...
turb_filt = turb_filt[turb_filt['Turbidity_NTU'] < turb_threshold]
turb_filt = turb_filt[turb_filt['Turbidity_NTU'] > turb_min]
...
if adcp_fit_type == 'exp':
    ...
    adcp_fit_eq = 'y = {:.5f} * e^({:.5f} * x)'.format(adcp_coeffs[0],adcp_coeffs[1]/10)
...
if turb_fit_type == 'thrd':
    turb_coeffs, turb_cov = opt.curve_fit(thrd_func, turb_perc, isco_perc)
    turb_ffit_x = np.concatenate(([0],turb_perc,[np.max(turb_filt.values)]),axis=0)
