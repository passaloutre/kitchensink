    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_caltrackgui.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * caltrackgui.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * caltrackgui.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * caltrackgui (application created by MATLAB Compiler)
        * run_caltrackgui.sh (shell script that can be run to 
    ...
    All Platforms:
        * caltrackgui.dll (shared library, file extension varies by 
          platform)
        * caltrackgui.h (library header file)
        * caltrackgui.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * caltrackgui.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * caltrackgui.xml (documentation files)
        * caltrackgui.pdb (program database file - if DEBUG option 
          selected)
        * caltrackgui.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * caltrackgui.ctf (Component Technology File archive, contains 
          compiled M-code)
        * caltrackgui_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * caltrackgui.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * caltrackgui.ctf (Component Technology File archive, contains 
          compiled M-code)
        * caltrackgui.xla (Optionally created and placed in
          D:\mcode\caltrack\caltrackgui\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
