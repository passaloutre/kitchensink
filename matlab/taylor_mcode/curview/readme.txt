    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_curview_bm.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * curview_bm.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * curview_bm.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * curview_bm (application created by MATLAB Compiler)
        * run_curview_bm.sh (shell script that can be run to 
    ...
    All Platforms:
        * curview_bm.dll (shared library, file extension varies by 
          platform)
        * curview_bm.h (library header file)
        * curview_bm.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * curview_bm.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * curview_bm.xml (documentation files)
        * curview_bm.pdb (program database file - if DEBUG option 
          selected)
        * curview_bm.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * curview_bm.ctf (Component Technology File archive, contains 
          compiled M-code)
        * curview_bm_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * curview_bm.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * curview_bm.ctf (Component Technology File archive, contains 
          compiled M-code)
        * curview_bm.xla (Optionally created and placed in
