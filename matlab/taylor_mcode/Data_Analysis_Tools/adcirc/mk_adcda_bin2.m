...
% byte offsets
nbyte63=np*nbyte;   % eta bytes in binary 63 file
nbyte64=np*2*nbyte; %(u+v) bytes in binary 64 file
nbyte_eta=4*np+8; %(eta+time) bytes in DA file
nbyte_uv=4*(np*2-1); %skip for uv (minus 4bytes)
nbyte_euv=4*(np*3)+8; %skip for combined eta,u,v,time for one snapshot (DA file)
nbyte_hdr=12;  %header bytes in DA file
...
fprintf(1,'Total Timesteps: %g\nLoading Timestep (Cycle 1): %s',nt,'     ');
zz=0; %dummy value to write for uv
for k = 1:nt,
    ...
    fwrite(fid,zz,'float32',nbyte_uv); %placeholder for velocity
end
fprintf(1,'\nLoading Timestep (Cycle 2): %s','     ');
for k=1:nt
    fprintf(1,'\b\b\b\b\b%5.0f',k);
    %read 64
    ...
    fseek(fid,nbyte_hdr+(k-1)*nbyte_euv+nbyte_eta,'bof');
