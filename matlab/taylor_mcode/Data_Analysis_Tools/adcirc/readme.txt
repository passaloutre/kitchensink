 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_mk_adcda.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* mk_adcda.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* mk_adcda.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* mk_adcda (application created by MATLAB Compiler)
		* run_mk_adcda.sh (shell script that can be run 
	...
	All Platforms:            
		* mk_adcda.dll (shared library, file extension 
		  varies by platform)
		* mk_adcda.h (library header file)
		* mk_adcda.ctf (Component Technology File archive,
	...
	* mk_adcda.xml (documentation files)
	* mk_adcda.pdb (program database file - if 
	  DEBUG option selected)
	* mk_adcda.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* mk_adcda.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* mk_adcda_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* mk_adcda.jar (Java class files and compiled M-code)
	...
	* mk_adcda.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* mk_adcda.xla (Optionally created and placed in
